...
#define MQTT_MAX_PACKET_SIZE 1024 //768
// --------------------------------------------------------------------
    ...
    if (simple ^ hasACState((decode_type_t)i)) {
      switch (i) {
        ...
        case decode_type_t::GLOBALCACHE:
            //debug("htmlSelectAcStateProtocol, default switch , windfat hack 1");
          break;
        default:
          // windfat hack
          //debug("htmlSelectAcStateProtocol, default switch , windfat hack");
...
void handleExamples(void) {
  ...
  html += F(
    ...
    "<p><a href=\"ir?" KEY_CODE "=0000,006E,0022,0002,0155,00AA,0015,0040,0015,"
        ...
        "0040,0015,0015,0015,0040,0015,0015,0015,0015,001listOfCommandTopics5,0E40"
...
#if SEALION_CONNECT_ENABLE
  String RegNum = String(newDeviceRegNum);
  char testdata[100];
  html += htmlButton(
            kUrlSendIoTDiscovery, F("Join Sealion IoT connect"), F("Send MQTT Discovery to Sealion IoT"));
         
  html += F("<p> Please input the following register number for IoT device discovery: ");
  html += RegNum;
  html += F("</p>");
...
bool parseStringAndSendRaw(IRsend *irsend, const String str) {
  ...
  // least two values. If not, bail out.setup_wifi
...
void handleSealionIoTDiscovery(void) {
    ...
    /* generate secret number between 1 and 100: */
      iSecret = rand() % 100 + 1;
      newDeviceRegNum = iSecret;
      
      sprintf(tmpstr, "%d", iSecret);
      iSecretStr= String(tmpstr);

      MqttSealionIoTDiscovery = "/sealion/iot/sealionIoTDeviceDiscovery/newRegistration" ;
      server.send(200, "text/html",
      htmlHeader(F("Sending MQTT Discovery message")) +
      htmlMenu() +
      "<p> The Sealion IoT MQTT Discovery message is being sent to topic: " +
      MqttSealionIoTDiscovery + ". It will show up in Sealion IoT connect in a few seconds." + 
      "Please input this number:" + iSecretStr + " to Sealion discovery device page:"
      "</p>"
      "<h3>Warning!</h3>"
      "<p>Sealion IoT connect's config for this device is reset each time this is "
      " is sent.</p>" +
      addJsReloadUrl(kUrlRoot, kRebootTime, true) +
      htmlEnd());

      
      sendMQTTIoTDiscovery(MqttSealionIoTDiscovery.c_str(), String(tmpstr));
